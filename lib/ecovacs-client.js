	...
	async connect() {
		try {
			...
			try {
				...
				// 3. Named export .Ecovacs or .Client
			...
			if (typeof this.client.connect === 'function') {
				try {
					...
					if (this.debugFlags.auth) {
						this.adapter.log.debug('[DEBUG-AUTH] Connected successfully');
					}
				} catch (error) {
					this.adapter.log.warn(`Connection method failed: ${error.message}`);
				}
			} else if (typeof this.client.authenticate === 'function') {
				try {
					...
					this.adapter.log.warn(`Authentication method failed: ${error.message}`);
				}
			}
